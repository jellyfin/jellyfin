...
namespace MediaBrowser.Providers.MediaInfo
{
    ...
    public class AudioFileProber
    {
        ...
        private async Task FetchDataFromTags(Audio audio, Model.MediaInfo.MediaInfo mediaInfo, MetadataRefreshOptions options, bool tryExtractEmbeddedLyrics)
        {
            ...
            {
                audio.PremiereDate = track.OriginalReleaseDate.Value.ToLocalTime();
            }
            else if (track.PublishingDate.HasValue && track.PublishingDate != DateTime.MinValue)
            {
                audio.PremiereDate = track.PublishingDate.Value.ToLocalTime();
            }
            else if (track.Date.HasValue)
            {
                audio.PremiereDate = track.Date.Value.ToLocalTime();
            }

            if (audio.PremiereDate.HasValue && audio.PremiereDate.Value != DateTime.MinValue)
            {
