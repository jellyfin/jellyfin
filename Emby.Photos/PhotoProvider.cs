...
public class PhotoProvider : ICustomMetadataProvider<Photo>, IForcedProvider, IHasItemChangeMonitor
{
    ...
    private static readonly FrozenSet<string> _includeExtensions = FrozenSet.ToFrozenSet(
        new[] { ".jpg", ".jpeg", ".png", ".tiff", ".cr2", ".webp", ".avif" },
    ...
    public Task<ItemUpdateType> FetchAsync(Photo item, MetadataRefreshOptions options, CancellationToken cancellationToken)
    {
        ...
        if (_includeExtensions.GetAlternateLookup<ReadOnlySpan<char>>().Contains(Path.GetExtension(item.Path.AsSpan())))
        {
            try
            {
