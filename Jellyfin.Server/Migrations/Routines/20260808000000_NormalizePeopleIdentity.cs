...
public class NormalizePeopleIdentity : IAsyncMigrationRoutine
{
    ...
    {
        // A credit belongs to the by-name item its kind resolves to: an Artist to the MusicArtist the
        // library browses music by, everything else to a Person.
        ...
        foreach (var credit in stored)
        {
            ...
            {
                // Created here so every credit leaves the migration linked, rather than waiting for the
                // first validation. Null when the folder cannot be created; the credit stays unlinked.
