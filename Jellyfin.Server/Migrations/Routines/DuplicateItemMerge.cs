...
/// <summary>
/// The two steps every migration that folds one BaseItem into another needs: repointing what references the duplicate, and deleting the duplicates afterwards.
/// </summary>
internal static class DuplicateItemMerge
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var offset = 0; offset < ids.Count; offset += DeleteBatchSize)
        {
            ...
            // Resolve via LibraryManager so DeleteItemsUnsafeFast can also remove the by-name
            // metadata folders the duplicate stubs left behind, and fall back to the persistence
            // service for the ones it cannot resolve.
    ...
    public static async Task<Dictionary<Guid, int>> CountReferencesAsync(
        ...
        CancellationToken cancellationToken)
    {
        // Well under SQLite's variable limit, so a candidate set of any size costs a handful of queries.
