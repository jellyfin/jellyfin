...
namespace Common.Networking
{
    ...
    public class InternetChecker : IDisposable
    {
        ...
        {
            _timer = new Timer(CheckInternetAccess, null, TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(10));
        }
        ...
        private void ConfigChanged(object sender, EventArgs args)
        {
            ...
            int check = 10; //// _config.Configuration.CheckEvery;
        ...
        /// Singleton accessor.
        /// </summary>
        public static InternetChecker Instance = new InternetChecker();
        ...
        private async Task<InternetState> TryInternetAccess()
        {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (!string.IsNullOrEmpty(_externalRedirector))
                    {
                        ...
                        try
                        {
                            ...
                            if (string.Equals(content, FileVersionInfo.GetVersionInfo(Assembly.GetEntryAssembly().Location).ProductName))
                            {
                                // Rocking and rolling!                                
            ...
            _logger?.LogInformation("Internet access is up.");            
            return InternetState.Up;
        }
