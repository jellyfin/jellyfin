...
namespace MediaBrowser.Api
{
    ...
    public class ItemUpdateService : BaseApiService
    {
        ...
        private void UpdateItem(BaseItemDto request, BaseItem item)
        {
            ...
            foreach (var (key, value) in request.ProviderIds.ToList())
            {
                if (string.IsNullOrEmpty(value))
                {
                    request.ProviderIds.Remove(key);
                }
            }
            ...
            if (request.AlbumArtists != null && item is IHasAlbumArtist hasAlbumArtists)
            {
            ...
            if (request.ArtistItems != null && item is IHasArtist hasArtists)
            {
