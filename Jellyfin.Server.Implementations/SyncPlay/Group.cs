...
using Jellyfin.Api.Models.SyncPlay.GroupStates;
...
namespace Jellyfin.Server.Implementations.SyncPlay
{
    /// <summary>
    /// Class Group.
    /// </summary>
    ...
    public class Group : IGroupStateContext
    {
        ...
        /// </summary>
        /// <value>The group identifier.</value>
        public GroupPlaylist PlayQueue { get; } = new GroupPlaylist();
        ...
        /// <param name="filter">The filtering type.</param>
        ...
        /// <param name="session">The session.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public void SessionJoin(SessionInfo session, CancellationToken cancellationToken)
        {
            AddSession(session);
        ...
        public Task SendGroupUpdate<T>(SessionInfo from, SessionsFilterType filter, GroupUpdateDto<T> message, CancellationToken cancellationToken)
        {
            IEnumerable<Task> GetTasks()
            {
                ...
                {
                    yield return _sessionManager.SendSyncPlayMessage(session, message, cancellationToken);
                }
            }
        ...
        /// <inheritdoc />
        public Task SendPlaybackCommand(SessionInfo from, SessionsFilterType filter, PlaybackCommandDto message, CancellationToken cancellationToken)
        {
            IEnumerable<Task> GetTasks()
            {
                foreach (var session in FilterSessions(from, filter))
