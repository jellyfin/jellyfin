    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            for (var index = 0; index < collections.Length; index++)
            {
                ...
                await CleanupLinkedChildren(collection, cancellationToken).ConfigureAwait(false);
        ...
        for (var index = 0; index < playlists.Length; index++)
        {
            ...
            await CleanupLinkedChildren(playlist, cancellationToken).ConfigureAwait(false);
            progress.Report(50D / playlists.Length * (index + 1));
        }
    }

    private async Task CleanupLinkedChildren<T>(T folder, CancellationToken cancellationToken)
        where T : Folder
    {
