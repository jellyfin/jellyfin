...
{
    /// <summary>
    /// Defines the <see cref="StreamBuilder" />.
    /// </summary>
    public class StreamBuilder
    {
        ...
        /// <summary>
        /// Normalizes the media source format into a single container.
        /// </summary>
        ...
        /// <summary>
        /// Gets the subtitle profile.
        /// </summary>
        /// <param name="mediaSource">The <see cref="MediaSourceInfo"/>.</param>
        /// <param name="subtitleStream">The <see cref="MediaStream"/>.</param>
        /// <param name="subtitleProfiles">An array of <see cref="SubtitleProfile"/>.</param>
        /// <param name="playMethod">The <see cref="PlayMethod"/>.</param>
        /// <param name="transcoderSupport">The <see cref="ITranscoderSupport"/>.</param>
        /// <param name="outputContainer">The output container.</param>
        /// <param name="transcodingSubProtocol">Optional transcoding sub protocol.</param>
        /// <returns>The <see cref="SubtitleProfile"/>.</returns>
            ...
            string? transcodingSubProtocol)
        {
            ...
            if (transcoderSupport == null)
            {
                throw new ArgumentNullException(nameof(transcoderSupport));
            }

            if (subtitleProfiles == null)
            {
                throw new ArgumentNullException(nameof(subtitleProfiles));
            }
            ...
            if (!subtitleStream.IsExternal && (playMethod != PlayMethod.Transcode || !string.Equals(transcodingSubProtocol, "hls", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                foreach (var profile in subtitleProfiles)
                {
                    ...
                    if (outputContainer != null && !ContainerProfile.ContainsContainer(profile.Container, outputContainer))
                    {
                        continue;
                    }
            ...
            return GetExternalSubtitleProfile(mediaSource, subtitleStream, subtitleProfiles, playMethod, transcoderSupport, false) ??
                ...
                new SubtitleProfile(subtitleStream.Codec, SubtitleDeliveryMethod.Encode);
        }
        ...
        private static IOrderedEnumerable<StreamInfo> SortMediaSources(List<StreamInfo> streams, long maxBitrate)
        {
            ...
            {
                return i.PlayMethod switch
                {
                    // Let's assume direct streaming a file is just as desirable as direct playing a remote url
                    PlayMethod.DirectStream or PlayMethod.DirectPlay => 0,
                    _ => 1,
                };
            }).ThenBy(i =>
            {
                return i.MediaSource?.Protocol switch
                {
            ...
            }).ThenBy(i =>
            {
                if (maxBitrate > 0)
                {
                    if ((i.MediaSource != null) && (i.MediaSource.Bitrate != null))
                    {
        ...
        private static TranscodeReason? GetTranscodeReasonForFailedCondition(ProfileCondition condition)
        {
            ...
            {
                ProfileConditionValue.AudioBitrate => TranscodeReason.AudioBitrateNotSupported,
                ProfileConditionValue.AudioChannels => TranscodeReason.AudioChannelsNotSupported,
                ProfileConditionValue.AudioProfile => TranscodeReason.AudioProfileNotSupported,
                ProfileConditionValue.AudioSampleRate => TranscodeReason.AudioSampleRateNotSupported,
                ProfileConditionValue.Has64BitOffsets => null, // TODO
                ProfileConditionValue.Height => TranscodeReason.VideoResolutionNotSupported,
                ProfileConditionValue.IsAnamorphic => TranscodeReason.AnamorphicVideoNotSupported,
                ProfileConditionValue.IsAvc => null, // TODO
                ProfileConditionValue.IsInterlaced => TranscodeReason.InterlacedVideoNotSupported,
                ProfileConditionValue.IsSecondaryAudio => TranscodeReason.SecondaryAudioNotSupported,
                ProfileConditionValue.NumAudioStreams => null, // TODO
                ProfileConditionValue.NumVideoStreams => null, // TODO
                ProfileConditionValue.PacketLength => null, // TODO
                ProfileConditionValue.RefFrames => TranscodeReason.RefFramesNotSupported,
                ProfileConditionValue.VideoBitDepth => TranscodeReason.VideoBitDepthNotSupported,
                ProfileConditionValue.AudioBitDepth => TranscodeReason.AudioBitDepthNotSupported,
                ProfileConditionValue.VideoBitrate => TranscodeReason.VideoBitrateNotSupported,
                ProfileConditionValue.VideoCodecTag => TranscodeReason.VideoCodecNotSupported,
                ProfileConditionValue.VideoFramerate => TranscodeReason.VideoFramerateNotSupported,
                ProfileConditionValue.VideoLevel => TranscodeReason.VideoLevelNotSupported,
                ProfileConditionValue.VideoProfile => TranscodeReason.VideoProfileNotSupported,
                ProfileConditionValue.VideoTimestamp => null, // TODO
                ProfileConditionValue.Width => TranscodeReason.VideoResolutionNotSupported,
                _ => null,
            };
        }
        ...
        private static void ApplyTranscodingConditions(StreamInfo item, IEnumerable<ProfileCondition> conditions, string? qualifier, bool enableQualifiedConditions, bool enableNonQualifiedConditions)
        {
        ...
        private StreamInfo? BuildVideoItem(MediaSourceInfo item, VideoOptions options)
        {
            ...
            // TODO: This doesn't account for situation of device being able to handle media bitrate, but wifi connection not fast enough
        ...
        private StreamInfo? BuildAudioItem(MediaSourceInfo item, AudioOptions options)
        {
            ...
            foreach (var i in options.Profile.TranscodingProfiles)
            {
                ...
                {
                    var audioCodec = i.AudioCodec ?? i.Container;
                    if (audioCodec != null && _transcoderSupport.CanEncodeToAudioCodec(audioCodec))
                    {
                        transcodingProfile = i;
                        break;
                    }
