...
public class ProviderManagerTests
{
    ...
    public static TheoryData<int, int[]?, int[]?, int?[]?, int[]> GetImageProvidersOrderData()
        => new()
        {
            { 3, null, null, null, [0, 1, 2] }, // no order options set

            // library options ordering
            { 3, [], null, null, [0, 1, 2] }, // no order provided
            { 3, [1], null, null, [1, 0, 2] }, // one item in order
            { 3, [2, 1, 0], null, null, [2, 1, 0] }, // full reverse order

            // server options ordering
            { 3, null, [], null, [0, 1, 2] }, // no order provided
            { 3, null, [1], null, [1, 0, 2] }, // one item in order
            { 3, null, [2, 1, 0], null, [2, 1, 0] }, // full reverse order

            // IHasOrder ordering
            { 3, null, null, [null, 1, null], [1, 0, 2] }, // one item with defined order
            { 3, null, null, [2, 1, 0], [2, 1, 0] }, // full reverse order

            // multiple orders set
            { 3, [1], [2, 0, 1], null, [1, 0, 2] }, // partial library order first, server order ignored
            { 3, [1], null, [2, 0, 1], [1, 2, 0] }, // library order first, then orderby
            { 3, [2, 1, 0], [1, 2, 0], [2, 0, 1], [2, 1, 0] }, // library order wins
        };
        ...
        bool baseItemEnabled = true)
    {
        ...
        AddParts(providerManager, imageProviders: [provider]);
    ...
    public static TheoryData<string[], int[]?, int[]?, int[]?, int[]?, int?[]?, int[]> GetMetadataProvidersOrderData()
    {
        ...
        return new()
        {
            { [l, l, r, r], null, null, null, null, null, [0, 1, 2, 3] }, // no order options set

            // library options ordering
            { [l, l, r, r], [], [], null, null, null, [0, 1, 2, 3] }, // no order provided
            // local only
            { [r, l, l, l], [2], null, null, null, null, [2, 0, 1, 3] }, // one item in order
            { [r, l, l, l], [3, 2, 1], null, null, null, null, [3, 2, 1, 0] }, // full reverse order
            // remote only
            { [l, r, r, r], null, [2], null, null, null, [2, 0, 1, 3] }, // one item in order
            { [l, r, r, r], null, [3, 2, 1], null, null, null, [3, 2, 1, 0] }, // full reverse order
            // local and remote, note that results will be interleaved (odd but expected)
            { [l, l, r, r], [1], [3], null, null, null, [1, 3, 0, 2] }, // one item in each order
            { [l, l, l, r, r, r], [2, 1, 0], [5, 4, 3], null, null, null, [2, 5, 1, 4, 0, 3] }, // full reverse order

            // // server options ordering
            { [l, l, r, r], null, null, [], [], null, [0, 1, 2, 3] }, // no order provided
            // local only
            { [r, l, l, l], null, null, [2], null, null, [2, 0, 1, 3] }, // one item in order
            { [r, l, l, l], null, null, [3, 2, 1], null, null, [3, 2, 1, 0] }, // full reverse order
            // remote only
            { [l, r, r, r], null, null, null, [2], null, [2, 0, 1, 3] }, // one item in order
            { [l, r, r, r], null, null, null, [3, 2, 1], null, [3, 2, 1, 0] }, // full reverse order
            // local and remote, note that results will be interleaved (odd but expected)
            { [l, l, r, r], null, null, [1], [3], null, [1, 3, 0, 2] }, // one item in each order
            { [l, l, l, r, r, r], null, null, [2, 1, 0], [5, 4, 3], null, [2, 5, 1, 4, 0, 3] }, // full reverse order

            // IHasOrder ordering (not interleaved, doesn't care about types)
            { [l, l, r, r], null, null, null, null, [2, null, 1, null], [2, 0, 1, 3] }, // partially defined
            { [l, l, r, r], null, null, null, null, [3, 2, 1, 0], [3, 2, 1, 0] }, // full reverse order

            // multiple orders set
            { [l, l, l, r, r, r], [1], [4], [2, 1, 0], [5, 4, 3], null, [1, 4, 0, 2, 3, 5] }, // partial library order first, server order ignored
            { [l, l, l], [1], null, null, null, [2, 0, 1], [1, 2, 0] }, // library order first, then orderby
            { [l, l, l, r, r, r], [2, 1, 0], [5, 4, 3], [1, 2, 0], [4, 5, 3], [5, 4, 1, 6, 3, 2], [2, 5, 4, 1, 0, 3] }, // library order wins (with orderby between local/remote)
        };
    }
        ...
        bool ownedItem = false)
    {
        ...
        using var providerManager = GetProviderManager(baseItemManager: baseItemManager.Object);
        AddParts(providerManager, metadataProviders: [provider]);
        ...
        string[]? metadataFetcherOrder = null)
    {
        ...
        if (imageFetcherOrder is not null || localMetadataReaderOrder is not null || metadataFetcherOrder is not null)
        {
            ...
            serverConfiguration.MetadataOptions =
            [
                new MetadataOptions
                {
                    ItemType = typeName,
