...
public class SafeNameHelperTests
{
    ...
    {
        var deepBaseDir = Path.Combine(
            Path.GetTempPath(),
            new string('d', SafeNameHelper.MaxFullPathLength - Path.GetTempPath().Length - 51));
    ...
    {
        var value = SafeNameHelper.MaxFullPathLength;

        if (OperatingSystem.IsWindows())
        {
            Assert.True(
                value > 0 && value <= 260,
                $"Expected Windows-safe path length ≤ 260, got {value}");
        }
        else
        {
            Assert.True(
                value > 0 && value <= 4096,
                $"Expected POSIX-safe path length ≤ 4096, got {value}");
        }
    }
}
