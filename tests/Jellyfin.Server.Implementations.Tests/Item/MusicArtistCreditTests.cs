...
public sealed class MusicArtistCreditTests : IDisposable
{
    ...
    public MusicArtistCreditTests()
    {
        ...
        using (var ctx = CreateDbContext())
        {
            ...
            // A second release he only guests on, and a third he has nothing to do with.
            ctx.BaseItems.Add(new BaseItemEntity
            {
                Id = _guestAlbumId,
                Type = itemTypeLookup.BaseItemKindNames[BaseItemKind.MusicAlbum],
        ...
        // The default mock returns an empty queryable, which would make every count zero.
    ...
    {
        // The credit link never looks at the target's type, so the query that finds a person's
        // filmography finds an artist's discography.
    ...
    {
        // Renaming rewrites CleanName, which is what the old ItemValues path matched on.
        using (var ctx = CreateDbContext())
        {
    ...
    {
        // The distinction AlbumArtist exists for, and the one a compilation depends on.
    ...
    {
        // Sorting reads the credit, so it has to survive the rename that used to strand it.
    ...
    {
        // The bug this is for: a clean name that stops matching drops the artist out of /Artists.
        Rename("Miles Dewey Davis III");
    ...
    {
        // Nothing writes artist values any more, so one that outlives the conversion is stale data.
        // Pinned because this deletes what it matches.
        using (var ctx = CreateDbContext())
        {
            ctx.PeopleBaseItemMap.RemoveRange(ctx.PeopleBaseItemMap);
            ctx.Peoples.RemoveRange(ctx.Peoples);
    ...
    {
        // People validation queries Person only, so an artist item is out of its reach.
        var result = _repository.GetItemList(new InternalItemsQuery
        {
            IncludeItemTypes = [BaseItemKind.MusicArtist],
            IsDeadPerson = true
