...
public sealed class PeopleRepositoryUpdatePeopleTests : SqliteDbTestFixture
{
    ...
    public void UpdatePeople_CreditForAnotherPersonOfTheSameName_GetsItsOwnRow()
    {
        ...
        // Another human of the same name, credited elsewhere: never repointed, so it owns a row of
        // its own. On a second item, or dropping the first credit would take its row with it.
