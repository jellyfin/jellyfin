...
namespace MediaBrowser.XbmcMetadata.Providers
{
    ...
    public class AlbumNfoProvider : BaseNfoProvider<MusicAlbum>
    {
        ...
        /// <inheritdoc />
        protected override async Task Fetch(MetadataResult<MusicAlbum> result, string path, CancellationToken cancellationToken)
        {
            await new BaseNfoParser<MusicAlbum>(_logger, _config, _providerManager, _userManager, _userDataManager, _genreManager, _directoryService).Fetch(result, path, cancellationToken).ConfigureAwait(false);
        }
