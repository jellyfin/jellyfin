    ...
    public static class Program
    {
        private static async Task<int> Main(string[] args)
        {
            return await Parser.Default.ParseArguments<Options.Wizard, object>(args)
                .MapResult(
                    async (Options.Wizard options) => await RunWizard(options),
        ...
        private static async Task<int> RunWizard(Options.Wizard options)
        {
            ...
            var firstUser = await startupController.GetFirstUser();
            ...
            await startupController.UpdateStartupUser(config.GetStartupUserDto());
