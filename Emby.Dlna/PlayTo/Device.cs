...
namespace Emby.Dlna.PlayTo
{
    public class Device : IDisposable
    {
        ...
        {
            TransportCommands? avCommands = await GetAVProtocolAsync(cancellationToken).ConfigureAwait(false);
            if (avCommands == null)
            {
                return;
            }
            ...
            await new SsdpHttpClient(_httpClient)
                .SendCommandAsync(
                    Properties.BaseUrl,
                    service,
                    command.Name,
        ...
        private async Task GetVolume(CancellationToken cancellationToken)
        {
            ...
            Volume = int.Parse(volumeValue, _usCulture);
        ...
        private async Task<(bool, uBaseObject?)> GetPositionInfo(TransportCommands avCommands, CancellationToken cancellationToken)
        {
            ...
            {
                Duration = TimeSpan.Parse(duration, _usCulture);
            }
            else
            {
                Duration = null;
            ...
            {
                Position = TimeSpan.Parse(position, _usCulture);
            }
            ...
            var uTrack = CreateUBaseObject(e, trackUri ?? string.Empty);
        ...
        private void UpdateMediaInfo(uBaseObject? mediaInfo, TransportState state)
        {
            ...
            if (mediaInfo != null && previousMediaInfo == null)
            {
            ...
            {
                if (!mediaInfo.Equals(previousMediaInfo))
                {
        ...
        private Task SetPlay(TransportCommands? avCommands, CancellationToken cancellationToken)
        {
            if (avCommands == null)
            {
                throw new InvalidOperationException("avCommands is null.");
            }
