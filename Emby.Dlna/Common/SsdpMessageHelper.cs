...
namespace Emby.Dlna.Net.Parsers
{
    ...
    public static class SsdpMessageHelper
    {
        ...
        public static string BuildMessage(string header, Dictionary<string, string> values)
        {
            ...
            var manager = DlnaSystemManager.Instance;
            if (manager != null)
            {
                values[manager.NetworkChangeCount + "-NLS"] = DlnaSystemManager.NetworkLocationSignature;
                values["OPT"] = SsdpOpt + manager.NetworkChangeCount;
                values["SERVER"] = DlnaSystemManager.Name;
            }
