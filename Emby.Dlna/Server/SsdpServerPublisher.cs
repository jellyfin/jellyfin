...
namespace Emby.Dlna.Server
{
    ...
    public class SsdpServerPublisher : IDisposable
    {
        ...
        private readonly string[] ssdpMulticastAddress = { "239.255.255.250:1900", "[ff02::C]:1900", "[ff05::C]:1900" };
        ...
        private Task SendAliveNotification(SsdpDevice device, string notificationType, string uniqueServiceName)
        {
            ...
            for (int a = begin; a <= end; a++)
            {
                ...
                if (_socketServer.Tracing)
                {
                    _logger.LogDebug("-> NOTIFY ssdp:alive : {0} -> {1} : {2}", rootDevice.Address, ssdpMulticastAddress[a], message);
                }
            ...
            for (int a = begin; a <= end; a++)
             {
                ...
                if (_socketServer.Tracing)
                {
                    _logger.LogDebug("->NOTIFY ssdp:byebye {0} : {1}", addr, ssdpMulticastAddress[a]);
                }
