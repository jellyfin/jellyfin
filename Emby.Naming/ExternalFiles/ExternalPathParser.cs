...
namespace Emby.Naming.ExternalFiles
{
    ...
    public class ExternalPathParser
    {
        ...
        public ExternalPathParserResult? ParseFile(string path, string? extraString)
        {
            ...
            if (!(_type == DlnaProfileType.Subtitle && _namingOptions.SubtitleFileExtensions.Contains(extension, StringComparer.OrdinalIgnoreCase))
                && !(_type == DlnaProfileType.Audio && _namingOptions.AudioFileExtensions.Contains(extension, StringComparer.OrdinalIgnoreCase))
                && !(_type == DlnaProfileType.Lyric && _namingOptions.LyricFileExtensions.Contains(extension, StringComparer.OrdinalIgnoreCase)))
            {
                return null;
            }
